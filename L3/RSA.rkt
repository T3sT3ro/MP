...
(define (solve-ax+by=1 a b)
        {if [= b 0]
            (cons 1 0)
            (let ([x (solve-ax+by=1 b (modulo a b))])
                 {cons (cdr x) (- (car x) (* (floor (/ a b)) (cdr x)))})
        }
)
...
;;; Data structure that represents signed messages here
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
        {let ([encrypted (RSA-encrypt message recipient-public-key)]) ;; encrypted returns list of numbers
             {cons (RSA-convert-list (list (compress encrypted)) sender-private-key) ;; signature is hash as single element list passed to RSA
                   encrypted}
        }
)

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
        {let ([sender-signature (car (RSA-unconvert-list (get-signature cyphertext) sender-public-key))]
              [ciphered-msg (get-message cyphertext)])
          
             {if [= sender-signature (compress (get-message cyphertext))]
                 (RSA-decrypt ciphered-msg recipient-private-key)
                 (error "Signature doesn't match, beware of scam!")}
        }
)
